...
export default defineConfig({
  ...
  // 注意：Ant Design Pro v6 默认使用 Rust 版 utoopack 打包器，但其
  // Windows 原生二进制在本机加载失败(ERR_DLOPEN_FAILED)，因此禁用
  // utoopack(注释掉下方配置即可)，umi 会自动回退到稳定的 webpack。
  // utoopack: {
  //   module: { rules: { '*.md': { loaders: [{ loader: join(__dirname, 'md-raw-loader.cjs') }], as: '*.js' } } },
  // },
  // webpack 下将 .md 作为原始字符串引入(替代 utoopack 的 md-raw-loader)
