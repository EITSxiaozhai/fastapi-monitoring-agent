...
const MachineCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <Row style={{ marginTop: 14, color: '#8c8c8c', fontSize: 12 }} gutter={8}>
        <Col span={8}>
        ...
        <Col span={8}>
          <Tooltip title={`内核 ${agent.kernel}`}>
            运行 <b style={{ color: 'rgba(0,0,0,0.85)' }}>{fmtUptime(agent.uptime_seconds)}</b>
          </Tooltip>
        </Col>
      </Row>
    </Card>
...
const Machines: React.FC = () => {
  ...
  const [metricField, setMetricField] = useState<'cpu' | 'mem'>('cpu');
  ...
  const historyData = useMemo(
    () =>
      history.map((p) => ({
        ...
        value: metricField === 'cpu' ? p.cpu_percent : p.mem_percent,
      })),
    [history, metricField],
  );
        ...
        {selected && (
          <>
            <StatisticCard.Group direction="row" style={{ marginBottom: 16 }}>
              ...
              <StatisticCard statistic={{ title: '负载', value: selected.load1 }} />
            </StatisticCard.Group>
            ...
            <Card size="small" style={{ marginBottom: 16 }}>
              <Row gutter={[8, 8]} style={{ fontSize: 13 }}>
                ...
                <Col span={12}>Agent ID：{selected.agent_id}</Col>
              </Row>
            </Card>
            ...
            <div style={{ marginBottom: 12 }}>
              <Segmented
                value={metricField}
                onChange={(v) => setMetricField(v as 'cpu' | 'mem')}
                options={[
